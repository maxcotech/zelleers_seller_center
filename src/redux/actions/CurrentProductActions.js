...
export const uploadMainProductImage = (image_file, setILoading = null) => {
  return (dispatch, getState) => {
    ...
    if (state.current_product?.id !== null)
      fdata.append("product_id", state.current_product.id);
    if (state.current_product?.product_image !== null)
      fdata.append("old_image_url", state.current_product.product_image);
    axios
      .post(`${BASE_URL}product/image`, fdata)
      .then((result) => {
        ...
        if (result.data?.status === "success") {
          ...
          dispatch(
            setProductAttribute(
              result.data.data?.image_full_path,
              "product_image"
            )
          );
        } else {
      ...
      .catch((ex, trh) => {
        ...
        if (!ex?.response) {
          toast.error(
            "Please check your network and also avoid uploading heavy images",
            { toastId: "HeavyImage" }
          );
        } else {
          handleAxiosError(ex);
...
export const uploadGalleryImage = (image_file, label, setILoading) => {
  return (dispatch, getState) => {
    ...
    fdata.append("store_id", state.store.current_store?.id);
    if (state.current_product[label] !== null)
      fdata.append("old_image_url", state.current_product[label]);
    if (state.current_product["id"] !== null)
      fdata.append("product_id", state.current_product["id"]);
    setILoading ? setILoading(true) : dispatch(setLoading(true));
    axios
      .post(`${BASE_URL}product/gallery_image`, fdata)
      .then((result) => {
        setILoading ? setILoading(false) : dispatch(setLoading(false));
        if (result.data?.status === "success") {
          dispatch(
            setProductAttribute(result.data.data?.image_full_path, label)
          );
        } else {
          toast.error(result.data?.message ?? "An Error Occurred");
        }
...
export const uploadProductVariationImage = (
  file,
  variation,
  iloading = null,
  onComplete = null
  ...
  return (dispatch, getState) => {
    ...
    if (
      typeof state.current_product?.id !== "undefined" &&
      state.current_product?.id !== null
    ) {
      formData.append("product_id", state.current_product?.id);
    }
    if (state.store.current_store?.id !== null)
      formData.append("store_id", state.store.current_store?.id);
    if (variation.variation_image_url !== "")
      formData.append("old_image_url", variation.variation_image_url);
    if (typeof variation.id !== "undefined")
      formData.append("variation_id", variation.id);
    axios
      .post(`${BASE_URL}product/variation_image`, formData)
      .then((result) => {
