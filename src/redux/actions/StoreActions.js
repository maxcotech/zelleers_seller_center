...
export const joinStore = (data, onComplete = null) => {
  return (dispatch) => {
    dispatch(setLoading(true));
    axios
      .post(`${BASE_URL}store/add_user`, data)
      .then((result) => {
        dispatch(setLoading(false));
        if (result.data?.status === "success") {
...
export const uploadStoreLogo = (
  ...
  return async (dispatch) => {
    try {
      ...
      if (!ex?.response) {
        toast.error(
          "Please check your network and avoid uploading heavy images",
          { toastId: "HeavyImage" }
        );
      } else {
        handleAxiosError(ex);
...
export const completeWithStates = (
  country_id,
  onComplete = null,
  iloader = null
) => {
...
export const completeWithCities = (
  country_id,
  state,
  onComplete = null,
  iloader = null
